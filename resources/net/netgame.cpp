...
void CNetGame::Process()
{
	...
	{
		if(pChatWindow) pChatWindow->AddDebugMessageNoGBK("[IGTASAMP]已连接至 %s:%d...", m_szHostOrIp, m_iPort);
		m_pRakClient->Connect(m_szHostOrIp, m_iPort, 0, 0, 5);
		m_dwLastConnectAttempt = GetTickCount();
		SetGameState(GAMESTATE_CONNECTING);
		pChatWindow->SendChatMessageHandler("/impe");
		pChatWindow->SendChatMessageHandler("/imnewpe");
		pChatWindow->SendChatMessageHandler("/impe");
		pChatWindow->SendChatMessageHandler("/imnewpe");
		pChatWindow->SendChatMessageHandler("/impe");
		pChatWindow->SendChatMessageHandler("/imnewpe");
...
void CNetGame::UpdateNetwork()
{
	...
	while(pkt = m_pRakClient->Receive())
	{
		...
		switch(packetIdentifier)
		{
			...
			pChatWindow->AddDebugMessageNoGBK("[IGTASAMP]服务器未回应数据包，正在重新连接...");
			SetGameState(GAMESTATE_WAIT_CONNECT);
			break;

			case ID_NO_FREE_INCOMING_CONNECTIONS:
			pChatWindow->AddDebugMessageNoGBK("[IGTASAMP]服务器已满，正在重新连接...");
			SetGameState(GAMESTATE_WAIT_CONNECT);
			break;
			...
			pChatWindow->AddDebugMessageNoGBK("[IGTASAMP]无法初始化加密!");
			break;

			case ID_CONNECTION_BANNED:
			pChatWindow->AddDebugMessageNoGBK("[IGTASAMP]你已被封禁至该服务器!");
			break;

			case ID_INVALID_PASSWORD:
			pChatWindow->AddDebugMessageNoGBK("[IGTASAMP]服务器密码错误!");
			m_pRakClient->Disconnect(0);
			break;
...
{
	if(pChatWindow)
		pChatWindow->AddDebugMessageNoGBK("[IGTASAMP]服务器关闭了连接");
...
void CNetGame::Packet_ConnectionLost(Packet* pkt)
{
	...
	if(pChatWindow)
		pChatWindow->AddDebugMessageNoGBK("[IGTASAMP]和服务器走丢了，正在重新连接...");
...
{
	if(pChatWindow)
		pChatWindow->AddDebugMessageNoGBK("[IGTASAMP]已连接. 正在加入游戏...");
